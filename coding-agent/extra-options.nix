...
let
  ...
  # Seatbelt, and what it is honestly worth.
  #
  # The Linux jail is closed by default: bwrap builds a new root and binds only
  # the runtime closure, so a path nobody named is not there at all. Writing the
  # same posture in SBPL would mean enumerating every read a toolchain makes,
  # and a profile that is too tight fails by refusing to start, on a machine
  # this repo cannot test from. So the profile is open for reads and closed for
  # writes, with the credential paths carved back out of reads.
  #
  # That is a real boundary. `sandbox-exec` is enforced by the kernel and covers
  # everything pi spawns, which is more than the classifier can promise, and a
  # command that writes outside the working directory is refused rather than
  # judged. It is still weaker than the jail, and the option's description says
  # so rather than letting the name imply parity.
  sandboxProfile =
    let
      subpath = p: ''(subpath "${p}")'';
      writable = [
      ...
      ++ map subpath sandbox.writablePaths;
    in
    pkgs.writeText "pi-sandbox.sb" ''
      ;; Generated by pi-nix. Seatbelt profile for pi on darwin.
      (version 1)
      (allow default)

      ;; Writes are denied everywhere and then granted back, so a path nobody
      ;; named is read-only rather than quietly writable.
      (deny file-write*)
      (allow file-write*
        ${lib.concatStringsSep "\n        " writable})

      ;; The store is immutable on the host too; denying writes to it turns a
      ;; confusing partial failure into an immediate one.
      (deny file-write* (subpath "/nix/store"))

      ${lib.optionalString (sandbox.unreadablePaths != [ ]) ''
        ;; Carved out of the open read policy. The kernel refuses these to pi
        ;; and to anything pi spawns, which is the half the classifier cannot
        ;; enforce.
        (deny file-read*
          ${lib.concatStringsSep "\n          " (map subpath sandbox.unreadablePaths)})
      ''}
      ${sandbox.profileExtra}
    '';

  # Wrapping cannot go through upstream's `finalPackage`: it is readOnly, and
  # it throws when jail.enable is set on a non-Linux host, so there is no seam
  # there for darwin. This is the package the two installers use instead, and
  # on Linux it is `finalPackage` untouched.
  sandboxedPackage =
    if !(sandbox.enable && pkgs.stdenv.hostPlatform.isDarwin) then
      cfg.finalPackage
    else
      pkgs.writeShellScriptBin "pi" ''
        # -D parameters, not string interpolation: the working directory is
        # only known at launch, and a profile that baked one in would confine
        # every session to whichever directory built it.
...
in
{
  options = lib.setAttrByPath optionPath {
    ...
    sandbox = {
      enable = lib.mkEnableOption ''
        Seatbelt confinement for pi on darwin.

        `jail.enable` is bubblewrap and Linux only; upstream's `finalPackage`
        throws outright if it is set anywhere else. That leaves darwin with no
        OS-level containment at all, which is the gap this closes: on a machine
        where the other agents are sandboxed, pi should not be the one that is
        not.

        Honest about its strength. The Linux jail is closed by default and
        binds only the runtime closure, so nothing is visible unless it was
        named. This profile is open for reads and closed for writes: writes are
      ...
      writablePaths = lib.mkOption {
        ...
        default = [ ];
        description = ''
          Absolute paths pi may write to, in addition to the working directory,
          `$PI_CODING_AGENT_DIR`, and the system temporary directories.

          A subpath, not a prefix match: `/Users/x/y` does not permit
          `/Users/x/y-other`.
        '';
        example = lib.literalExpression ''[ "/Users/joe/Development" ]'';
      };

      unreadablePaths = lib.mkOption {
        type = lib.types.listOf lib.types.str;
        default = [ ];
        description = ''
          Absolute paths pi may not read, carved out of the otherwise open read
          policy. Credential files belong here.

          The Linux side spells the same intent as `autoMode.deniedPaths`, which
          the classifier enforces in the agent. This is enforced by the kernel
          and applies to anything pi spawns, which is the difference worth
          having.
        '';
        example = lib.literalExpression ''[ "/Users/joe/.ssh" ]'';
      };
      ...
      profileExtra = lib.mkOption {
        ...
        default = "";
        description = ''
          Extra Seatbelt clauses appended verbatim, for a rule the options
          above cannot express. Written in the same SBPL the rest of the
          profile uses.
        '';
      };
      ...
      profile = lib.mkOption {
        ...
        internal = true;
        readOnly = true;
        description = "The rendered Seatbelt profile, or null when disabled.";
      };
    };

    installedPackage = lib.mkOption {
      type = lib.types.package;
      internal = true;
      readOnly = true;
      description = ''
        The package the module actually installs.

        `finalPackage` on Linux, untouched. On darwin with `sandbox.enable` it
        is that package wrapped in `sandbox-exec`. It exists because
        `finalPackage` is readOnly and throws on a non-Linux host when
        `jail.enable` is set, so there is no seam in it for a second sandbox.
      '';
    };
  ...
  config = lib.setAttrByPath optionPath {
    ...
    sandbox.profile = if sandbox.enable then sandboxProfile else null;
    installedPackage = sandboxedPackage;
